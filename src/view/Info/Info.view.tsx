...
const InfoView = (props: Props) => {
  ...
  return (
    <section className={classes["info"]}>
      ...
      <button className={classes['goback']} onClick={scrollToTop} style={{opacity: visible ? 1 : 0}}>
      ...
      <div className={classes["sections"]}>
        ...
        <div id="posilca" className={`${classes["info-block"]} ${classes["gap"]}`}>
          ...
          <ul className={classes['pack']}>
            <li>ваши контакты</li>
            <li>{'контакты получателя (его телефон, и, если требуется доп. отправка уже в стране назначения на другой адрес - например, в другой город - то адрес, индекс получателя и т. д.)'}</li>
          </ul>
          <p>
          ...
          <p className={classes['text']}>
            ❌ На самой сумке ничего писать (от кого / кому и т. д.) <span>НЕ</span> нужно.  <br />
            Отвечаем на вопрос “почему?”: когда водитель получит от вас контактные данные, он присвоит вашей посылке внутренний номер. Этот номер вы и назовете получателю вашей посылки. Вы можете также дополнительно сфотографировать 📸 свою сумку и показать ее фото получателю - но это совсем не обязательно. 
          </p>
            <p className={classes['text']}>
            📩 Конверты рекомендуется <span>НЕ</span> запечатывать, так как таможенники часто вскрывают любые конверты / сумки / пакеты и проч. 
          </p>
          <p className={classes['text']}>
            ❗Если вы планируете передать ДОКУМЕНТЫ 📂, то внимательно читайте абзац ниже.
          </p>
        </div>
